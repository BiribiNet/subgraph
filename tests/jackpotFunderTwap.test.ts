...
function emitTwapWindowUpdated(seconds: i32): void {
  ...
  event.parameters.push(
    new ethereum.EventParam("twapWindowSeconds", ethereum.Value.fromUnsignedBigInt(BigInt.fromI32(seconds)))
  );
...
function emitColdSlippageBpsUpdated(bps: i32): void {
  ...
  event.parameters = new Array<ethereum.EventParam>();
  event.parameters.push(
    new ethereum.EventParam("coldSlippageBps", ethereum.Value.fromUnsignedBigInt(BigInt.fromI32(bps)))
  );
...
describe("BRBJackpotFunder TWAP config", () => {
  ...
  test("TwapWindowUpdated updates the funder config singleton", () => {
    ...
    assert.fieldEquals("JackpotFunderConfig", CONFIG_KEY, "twapWindowSeconds", "1800");
  });

  test("ColdSlippageBpsUpdated updates the funder config singleton", () => {
    emitColdSlippageBpsUpdated(250);

    assert.fieldEquals("JackpotFunderConfig", CONFIG_KEY, "coldSlippageBps", "250");
  });
});
